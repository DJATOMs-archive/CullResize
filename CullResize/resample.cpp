...
static void resize_v_sseX_planar_16or32(BYTE* dst0, const BYTE* src0, int dst_pitch, int src_pitch, ResamplingProgram* program, int width, int target_height, int bits_per_pixel, const int* pitch_table, const void* storage)
{
  int filter_size = program->filter_size;
  //short* current_coeff = program->pixel_coefficient;
...
static void resizer_h_ssse3_generic_int16_float(BYTE* dst8, const BYTE* src8, int dst_pitch, int src_pitch, ResamplingProgram* program, int width, int height, int bits_per_pixel) {
...
FilteredResizeH::FilteredResizeH( PClip _child, double subrange_left, double subrange_width,
                                  int target_width, int zscale, int phase, int phase2, ResamplingFunction* func, IScriptEnvironment* env )
  ...
  filter_storage_luma(0), filter_storage_chroma(0)
{
  ...
  resampling_program_luma = func->GetResamplingProgram(vi.width, subrange_left, subrange_width, target_width, 1, 0, 0, env2);
  if (vi.IsPlanar() && !grey && !isRGBPfamily) {
...
FilteredResizeV::FilteredResizeV( PClip _child, double subrange_top, double subrange_height,
                                  int target_height, int zscale, int phase, int phase2, ResamplingFunction* func, IScriptEnvironment* env )
  : GenericVideoFilter(_child),
    resampling_program_luma(0), resampling_program_chroma(0),
    ...
    filter_storage_chroma_aligned(0), filter_storage_chroma_unaligned(0)
{
  ...
  // Create resampling program and pitch table
  resampling_program_luma  = func->GetResamplingProgram(vi.height, subrange_top, subrange_height, target_height, zscale, phase, phase2, env2);
  ...
  if (vi.IsPlanar() && !grey && !isRGBPfamily) {
    ...
    resampling_program_chroma = func->GetResamplingProgram(
                                  ...
                                  subrange_top    / div,
                                  subrange_height / div,
                                  target_height  >> shift, zscale, phase, phase2,
                                  env2);
...
PClip FilteredResize::CreateResizeH(PClip clip, double subrange_left, double subrange_width, int target_width, int zscale, int phase, int phase2,
                    ResamplingFunction* func, IScriptEnvironment* env)
{
  ...
  if (subrange_left == int(subrange_left) && subrange_width == target_width
   && subrange_left >= 0 && subrange_left + subrange_width <= vi.width) {
	...
	if (((int(subrange_left) | int(subrange_width)) & mask) == 0) {
		...
		return env->Invoke("Crop", AVSValue(sargs, 6)).AsClip();
	}
  }
  ...
  PClip result = clip;
  if (vi.IsYUY2()) {
	  result = env->Invoke("ConvertYUY2ToYV16", AVSValue({result}, 1)).AsClip();//new ConvertYUY2ToYV16(result,  env);
  }
  result = new FilteredResizeH(result, subrange_left, subrange_width, target_width, zscale, phase, phase2, func, env);
  if (vi.IsYUY2()) {
    result = env->Invoke("ConvertYV16ToYUY2", AVSValue({ result }, 1)).AsClip();//new ConvertYV16ToYUY2(result,  env);
  }
...
PClip FilteredResize::CreateResizeV(PClip clip, double subrange_top, double subrange_height, int target_height, int zscale, int phase, int phase2,
                    ResamplingFunction* func, IScriptEnvironment* env)
{
  const VideoInfo& vi = clip->GetVideoInfo();
  ...
  return new FilteredResizeV(clip, subrange_top, subrange_height, target_height, zscale, phase, phase2, func, env);
}


PClip FilteredResize::CreateResize(PClip clip, int target_width, int target_height, int zscale, int phase, int phase2, const AVSValue* args,
                   ResamplingFunction* f, IScriptEnvironment* env)
{
  ...
  {
      result = CreateResizeV(clip, subrange_top, subrange_height, target_height, zscale, phase, phase2, f, env);
      result = CreateResizeH(result, subrange_left, subrange_width, target_width, zscale, phase, phase2, f, env);
  }
  else
  {
      result = CreateResizeH(clip, subrange_left, subrange_width, target_width, zscale, phase, phase2, f, env);
      result = CreateResizeV(result, subrange_top, subrange_height, target_height, zscale, phase, phase2, f, env);
  }
  return result;
}
...
AVSValue __cdecl FilteredResize::Create_BilinearResize(AVSValue args, void*, IScriptEnvironment* env)
{
  ...
  return CreateResize(args[0].AsClip(), args[1].AsInt(), args[2].AsInt(), args[7].AsInt(1), args[8].AsInt(0), args[9].AsInt(0), &args[3], &f, env);
}
...
AVSValue __cdecl FilteredResize::Create_BicubicResize(AVSValue args, void*, IScriptEnvironment* env)
{
  ...
  return CreateResize(args[0].AsClip(), args[1].AsInt(), args[2].AsInt(), args[9].AsInt(1), args[10].AsInt(0), args[11].AsInt(0), &args[5], &f, env);
}
...
AVSValue __cdecl FilteredResize::Create_LanczosResize(AVSValue args, void*, IScriptEnvironment* env)
{
  ...
  return CreateResize(args[0].AsClip(), args[1].AsInt(), args[2].AsInt(), args[8].AsInt(1), args[9].AsInt(0), args[10].AsInt(0), &args[3], &f, env);
}
...
{
	AVS_linkage = vectors;
	env->AddFunction("CullBilinearResize", "cii[src_left]f[src_top]f[src_width]f[src_height]f[scale]i[phase]i[phase2]i", FilteredResize::Create_BilinearResize, 0);
	env->AddFunction("CullBicubicResize", "cii[b]f[c]f[src_left]f[src_top]f[src_width]f[src_height]f[scale]i[phase]i[phase2]i", FilteredResize::Create_BicubicResize, 0);
	env->AddFunction("CullLanczosResize", "cii[src_left]f[src_top]f[src_width]f[src_height]f[taps]i[scale]i[phase]i[phase2]i", FilteredResize::Create_LanczosResize, 0);
	env->AddFunction("CullLanczos4Resize", "cii[src_left]f[src_top]f[src_width]f[src_height]f[scale]i[phase]i[phase2]i", FilteredResize::Create_Lanczos4Resize, 0);
	env->AddFunction("CullBlackmanResize", "cii[src_left]f[src_top]f[src_width]f[src_height]f[taps]i[scale]i[phase]i[phase2]i", FilteredResize::Create_BlackmanResize, 0);
	env->AddFunction("CullSpline16Resize", "cii[src_left]f[src_top]f[src_width]f[src_height]f[scale]i[phase]i[phase2]i", FilteredResize::Create_Spline16Resize, 0);
	env->AddFunction("CullSpline36Resize", "cii[src_left]f[src_top]f[src_width]f[src_height]f[scale]i[phase]i[phase2]i", FilteredResize::Create_Spline36Resize, 0);
	env->AddFunction("CullSpline64Resize", "cii[src_left]f[src_top]f[src_width]f[src_height]f[scale]i[phase]i[phase2]i", FilteredResize::Create_Spline64Resize, 0);
	env->AddFunction("CullGaussResize", "cii[src_left]f[src_top]f[src_width]f[src_height]f[p]f[scale]i[phase]i[phase2]i", FilteredResize::Create_GaussianResize, 0);
	/**
	...
	**/
	...
	return "CullResize for Avisynth+";
}