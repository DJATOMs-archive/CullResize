...
ResamplingProgram* ResamplingFunction::GetResamplingProgram(int source_size, double crop_start, double crop_size, int target_size, int zscale, int phase, int phase2, IScriptEnvironment2* env)
{
  ...
  ResamplingProgram* program = new ResamplingProgram(fir_filter_size, source_size, target_size, crop_start, crop_size, zscale, phase, phase2, env);
  ...
  if (zscale < 1) {
	env->ThrowError("CullResize: illegal scale (%d)", zscale);
  }

   pos = crop_start + ((crop_size - (zscale * target_size)) / (zscale * target_size * 2));

   filter_step = min(double(target_size * zscale) / crop_size, 1.0);

   double pos0 = pos - pos_step * ((double)phase / zscale);
   double pos1 = pos - pos_step * ((double)(phase + phase2) / zscale);

  for (int i = 0; i < target_size; ++i) {
    // Clamp start and end position such that it does not exceed frame size
	double &rpos = (i & 1) ? pos1 : pos0;
	int end_pos = int(rpos + filter_support);
	...
	double ok_pos = clamp(rpos, 0.0, (double)(source_size - 1));
