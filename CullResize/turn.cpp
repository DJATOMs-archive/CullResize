...
void Turn::SetUVSource(int mod_h, int mod_v, IScriptEnvironment* env)
{
	...
	u_or_b_source = env->Invoke(isRGB ? "BToY8" : "UToY8", AVSValue({ child }, 1)).AsClip();// Y16 and Y32 capable
	v_or_r_source = env->Invoke(isRGB ? "RToY8" : "VToY8", AVSValue({ child }, 1)).AsClip();
    ...
    u_or_b_source = FilteredResize::CreateResize(u_or_b_source, uv_width, uv_height, 1, 0, 0, subs, &filter, env); // dunno
    v_or_r_source = FilteredResize::CreateResize(v_or_r_source, uv_width, uv_height, 1, 0, 0, subs, &filter, env);
